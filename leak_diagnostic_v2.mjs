...
async function leakData() {
    console.log('--- EXTRAYENDO DATOS VÍA EXCEPCIÓN (REINTENTO) ---');
...
BEGIN
    SELECT jsonb_build_object(
        ...
        'recent_plans', (SELECT coalesce(jsonb_agg(p), '[]'::jsonb) FROM (SELECT id, title, group_id, subject_id, period_id FROM public.lesson_plans ORDER BY created_at DESC LIMIT 10) p),
        'recent_groups', (SELECT coalesce(jsonb_agg(g), '[]'::jsonb) FROM (SELECT id, grade, section FROM public.groups LIMIT 10) g),
        'recent_subjects', (SELECT coalesce(jsonb_agg(s), '[]'::jsonb) FROM (SELECT id, custom_name, group_id FROM public.group_subjects LIMIT 20) s)
    ) INTO info;
