...
-- 3. DISABLE THE TRIGGER PERMANENTLY FOR NOW
-- If the trigger logic (handle_new_user) has a bug that crashes auth transaction, disabling it will fix login.
-- Since our seed script manually creates profiles, we don't need this trigger for demo users.
-- For new real users, we might need it, but let's fix the 500 first.
