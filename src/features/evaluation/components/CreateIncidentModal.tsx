...
import { X, Save, AlertTriangle, FileText, ChevronDown } from 'lucide-react'
    ...
    const handleSubmit = async (e: React.FormEvent) => {
        ...
        try {
            ...
            if (incident) {
                ...
                ;({ queued } = await saveOrQueue({
                    table: 'student_incidents',
                    op: 'update',
                    values,
                    match: { id: incident.id },
                    dedupeKey: `incident-update:${incident.id}`,
                    label: `Incidencia: ${formData.title}`,
                }))
            } else {
                // El id se genera aquí para que reintentar sin señal nunca duplique el reporte
                ;({ queued } = await saveOrQueue({
                    table: 'student_incidents',
                    op: 'insert',
                    rows: [{
            ...
            if (queued) {
                showToast('Sin conexión: el reporte quedó guardado en este dispositivo y se enviará al volver la señal.', 'info', 5000)
            }
            ...
            <div className="squishy-card bg-white shadow-2xl w-full max-w-2xl max-h-[90vh] overflow-hidden flex flex-col border-none animate-in zoom-in-95 duration-300">
                <div className="p-8 border-b border-slate-50 flex justify-between items-center bg-white z-10">
                    <div className="flex items-center gap-4">
                        ...
                        <div>
                            ...
                            <p className="text-[11px] text-slate-500 font-black uppercase tracking-widest mt-0.5">Gestión de convivencia</p>
                        </div>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-8 overflow-y-auto custom-scrollbar">
                    ...
                    <div className="group/field">
                        <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1 group-focus-within/field:text-indigo-500 transition-colors">Alumno</label>
                        <div className="relative">
                            <select aria-label="Alumno"
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                        <div className="group/field">
                            <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1 group-focus-within/field:text-indigo-500 transition-colors">Tipo de Reporte</label>
                            <div className="relative">
                        ...
                        <div className="group/field">
                            <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1 group-focus-within/field:text-indigo-500 transition-colors">Severidad / Impacto</label>
                            <div className="relative">
                                <select aria-label="Severidad / Impacto"
                    ...
                    <div className="group/field">
                        <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1 group-focus-within/field:text-indigo-500 transition-colors">Título del Reporte</label>
                    ...
                    <div className="group/field">
                        <label className="block text-[11px] font-black text-slate-500 uppercase tracking-widest mb-3 ml-1 group-focus-within/field:text-indigo-500 transition-colors">Descripción Detallada</label>
                    ...
                    <div className={`p-6 rounded-[2rem] border-4 transition-all duration-500 ${formData.has_commitment ? 'border-indigo-500 bg-indigo-50/30' : 'border-slate-50 bg-slate-50/30 hover:border-slate-100'}`}>
                        <div className="flex items-center justify-between mb-2">
                            <label htmlFor="has_commitment" className="flex items-center gap-4 cursor-pointer group/label">
                                <div className={`w-12 h-12 rounded-2xl flex items-center justify-center transition-all ${formData.has_commitment ? 'bg-indigo-600 text-white shadow-lg shadow-indigo-200' : 'bg-white text-slate-500 shadow-inner group-hover/label:bg-indigo-50 group-hover/label:text-indigo-400'}`}>
                                    <FileText className="w-6 h-6" />
                                </div>
                                <div>
                                    <p className="font-black text-slate-800 leading-none">Acta de Compromiso</p>
                                    <p className="text-[11px] text-slate-500 font-black uppercase tracking-widest mt-1">Generar documento legal</p>
                                </div>
                            </label>
                        ...
                        {formData.has_commitment && (
                            <div className="mt-6 space-y-4 animate-in slide-in-from-top-4 duration-500">
                                <div className="p-4 bg-white/60 rounded-2xl border border-indigo-100">
                                    <p className="text-[11px] text-indigo-500 font-black uppercase tracking-widest mb-3 leading-tight opacity-60">Compromisos específicos</p>
                                    <textarea
                                ...
                                <div className="flex items-center gap-2 px-2 py-1 bg-amber-50 rounded-lg">
                                    ...
                                    <p className="text-[11px] font-black uppercase tracking-widest text-amber-700">Requiere firmas de tutor y alumno</p>
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="flex justify-end gap-4 p-8 bg-slate-50 mt-auto">
                        <button
                            type="button"
                            onClick={onClose}
                            className="px-8 py-3 rounded-2xl text-[11px] font-black uppercase tracking-widest text-slate-500 hover:bg-slate-100 transition-all active:scale-95"
                        >
                            Cancelar
                        </button>
                        <button
                            type="submit"
                            disabled={loading}
                            className="btn-tactile px-10 py-3 rounded-2xl bg-slate-900 text-white font-black uppercase tracking-widest text-[11px] flex items-center gap-3 disabled:opacity-50"
                        >
                            {loading ? (
                                <>
