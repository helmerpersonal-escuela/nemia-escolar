                ...
                <div className="md:col-span-2 space-y-8">
                    ...
                    <div className="bg-white p-8 rounded-3xl shadow-xl shadow-slate-100 border border-slate-50">
                        <div className="flex justify-between items-center mb-6">
                            <div>
                                ...
                                <p className="text-xs font-bold text-slate-500 uppercase tracking-widest mt-1">Estatus actual de los grupos</p>
                            </div>
                            <div className="flex items-center gap-3">
                                <span className="flex items-center gap-1.5 px-3 py-1 bg-emerald-50 text-emerald-700 rounded-full text-[11px] font-black uppercase tracking-widest">
                                ...
                                {unattendedGroups.length > 0 && (
                                    <span className="flex items-center gap-1.5 px-3 py-1 bg-red-50 text-red-600 rounded-full text-[11px] font-black uppercase tracking-widest">
                        ...
                        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                            {loading ? (
                                <div className="col-span-full py-10 text-center text-slate-500 animate-pulse font-bold text-xs uppercase tracking-widest">Cargando monitoreo...</div>
                                ...
                                activeModules.map(module => (
                                    <div key={module.id} className={`p-4 rounded-2xl border transition-all ${!module.is_attended ? 'bg-red-50/50 border-red-100' : 'bg-slate-50/50 border-slate-100'}`}>
                                        <div className="flex justify-between items-start mb-2">
                                            <span className="text-[11px] font-black text-slate-500">{module.start_time.slice(0, 5)} - {module.end_time.slice(0, 5)}</span>
                                            <span className={`text-[11px] font-black uppercase tracking-widest px-2 py-0.5 rounded-md ${!module.is_attended ? 'text-red-700 bg-red-100' : 'text-emerald-700 bg-emerald-100'}`}>
                                        ...
                                        <p className="text-[11px] font-medium text-slate-500 mt-2 italic flex items-center gap-1">
                                ...
                                <div className="col-span-full py-10 text-center bg-slate-50 rounded-2xl border-2 border-dashed border-slate-100">
                                    <Clock className="w-10 h-10 mx-auto mb-2 opacity-20 text-slate-500" />
                                    <p className="font-bold text-xs uppercase tracking-widest text-slate-500">Sin módulos activos en este momento</p>
                                </div>
                            )}
                        </div>
                    </div>
                    ...
                    <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-slate-100 border border-slate-50">
                        ...
                        <div className="space-y-4">
                            {loading ? (
                                <div className="py-10 text-center text-slate-500 animate-pulse font-bold uppercase tracking-widest text-xs">Cargando incidencias...</div>
                            ) : incidents.length > 0 ? (
                                incidents.map(inc => (
                                    <IncidentRow
                                        key={inc.id}
                                ...
                                <div className="py-10 text-center text-slate-500 bg-slate-50 rounded-2xl border-2 border-dashed border-slate-100">
                ...
                <div className="space-y-6">
                    ...
                    <div className="bg-indigo-600 p-8 rounded-[2rem] text-white shadow-xl shadow-indigo-200">
                        ...
                        <div className="grid grid-cols-2 gap-4">
                            <div>
                                <p className="text-[11px] font-black text-indigo-200 uppercase tracking-widest leading-none">Ausencias</p>
                                <p className="text-3xl font-black mt-1">{substitutionStats.activeAbsences}</p>
                            </div>
                            <div>
                                <p className="text-[11px] font-black text-indigo-200 uppercase tracking-widest leading-none">Pendientes</p>
                    ...
                    <div className="bg-white p-8 rounded-[2rem] shadow-xl shadow-slate-100 border border-slate-50">
                        ...
                        <div className="space-y-4">
                            ...
                            <div className="p-4 bg-amber-50 rounded-2xl flex items-center justify-between">
                                ...
                                <span className="text-2xl font-black text-amber-700">{stats.todayLates}</span>
                            </div>
                        </div>
                    </div>
...
const IncidentRow = ({ name, group, type, time, status, onViewReport }: any) => {
    const typeColors: any = {
        ...
        SALUD: 'text-amber-700 bg-amber-50',
    ...
    return (
        <div className="flex items-center justify-between p-4 bg-slate-50/50 rounded-2xl hover:bg-slate-50 transition-all border border-transparent hover:border-slate-100 group">
            <div className="flex items-center gap-4">
                ...
                <div>
                    <h5 className="font-bold text-slate-900">{name} <span className="text-slate-500 ml-2">{group}</span></h5>
                    <div className="flex items-center gap-3 mt-1">
                        <span className={`text-[11px] font-black px-2 py-0.5 rounded-md ${typeColors[type] || 'text-slate-600 bg-slate-100'}`}>{type}</span>
                        <span className="text-[11px] text-slate-500 font-medium">{time}</span>
                    </div>
                </div>
            </div>
            <div className="flex items-center gap-4">
                <span className={`text-[11px] font-black px-2 py-1 rounded-full ${status === 'OPEN' ? 'text-red-500 bg-red-50' : 'text-slate-500 bg-slate-100'}`}>{status}</span>
                <button
